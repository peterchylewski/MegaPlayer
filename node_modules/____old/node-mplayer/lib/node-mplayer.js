...
var MPlayer = function(file, options) {
	...
	function _sendCommand(cmd) {
		if (_child !== null) {
			...
			_childBusy = true;
			_child.stdin.write(cmd + '\n');
			_child.stdout.once('data', function(buffer) {
	...
	this.play = function(file, options) {
		...
		_child = spawn('mplayer', args);
		_child.stdout.on('data', function(buffer) { console.log(buffer.toString()); } );
	}
   ...
   this.mute = function() {
	...
	this.setVolume = function(volume) {
		...
		if (typeof volume === 'string') {
			switch (volume) {
				case 'up': 		_volume = _volume + 5;	break;
				case 'down':	_volume = _volume - 5;	break;
			}
		} else { _volume = volume; }

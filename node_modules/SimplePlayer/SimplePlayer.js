...
var SimplePlayer = function() {
	...
	this.play = function(file, newPlayer) {
		...
		//_self.emit('valueChanged', 'path', file);
	...
	this.start = function(file, newPlayer) {
		...
		if (_mplayer !== undefined && _mplayer !== null && _mplayer.stdin !== undefined) {
			...
			if (_mplayer.stdin.readable === false || _mplayer.stdin.writable === false) {
				_mplayer.kill();
				_mplayer = undefined;
			}
		}
		
		if (newPlayer === true) { _mplayer.kill(); _mplayer = undefined; }
		...
		if (_mplayer === undefined || _mplayer === null) {
			...
			console.log('*************** new mplayer spawned ***************');
	...
	function _setVolume(volume, feedback) {
		console.log('setting volume to ', volume, feedback);
		var cmd = 'volume ' + _volume +  ' 1';
		if (feedback === true) {
			cmd = cmd + '\nget_property volume';
		}
		_sendCommand(cmd, feedback);
	}
	
	function _killAllPlayers() {
		console.log('--------------------------------SimplePlayer._killAllPlayers');
		_self.stop();
		var killall = spawn('killall mplayer');
		killall.stdout.on('data', function(buffer) {
			console.log(' killall buffer', buffer);
		});
	}
	...
	function _sendCommand(cmd, feedback) {
		...
		if (_mplayer !== undefined && _mplayer !== null && _mplayer.stdin.writable === true) {
		
			var result = _mplayer.stdin.write(cmd + '\n');
			//console.log('_mplayer write result', result);
	...
	function _decodePlayerMessage(buffer) {
		...
		if (buffer === undefined || buffer === null) {
			console.log('buffer is empty, aborting');
		}
		...
		var lines = _s.trim(raw).split('\n'),
			info = {}, ans, audio, volume, icy, eof, exitCode,
			key, value;
		...
		function decode(s) {
			...
			if (audio !== undefined && audio !== null) {
				...
				try {
					var	sampleRateInfo = a[0].match(/([0-9]*) (.*)/),
						channelInfo = a[1].match(/([0-9]*) (.*)/),
						bitsPerSampleInfo = a[2],
						bitRateInfo = a[3].match(/([0-9\.]*)\W([a-z]*)/);
				
					info['AUDIO'] = {
						raw: audio,
						sampleRate: 	parseInt(sampleRateInfo[1] | 0),
						sampleRateUnit: sampleRateInfo[2] | 'xxx',
						channels: 		parseInt(channelInfo[1]) | 0,
						bitsPerSample: 	bitsPerSampleInfo,
						bitRate: 		parseFloat(bitRateInfo[1] | 0),
						bitRateUnit: 	bitRateInfo[2] | 'yyy'
					};
				} catch(e) {
				...
				_self.emit('audio_info', info['AUDIO']);
			}
			
			// --------------- detect volume message
			
			volume = s.match(/^Volume: (.*)/);
			if (volume !== null) {
				var vol = volume[1];

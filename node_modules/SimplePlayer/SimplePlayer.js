...
var SimplePlayer = function() {
	...
	this.start = function(file) {
		...
		var options = ['-slave', file, '-volume', _volume, '-msglevel', 'statusline=6', '-msglevel', 'global=6'],
	...
	function startListeningToPlayer() {
	...
	function _decodePlayerMessage(buffer) {
		...
		var eof = s.match(/^EOF(.*)$/);
		if (eof !== null) {
			console.log('>>>>>>>EOF', eof[1]);
			_self.emit('end_of_file_reached');
			if (_mode === 'static') { _self.stop(); }
		}
